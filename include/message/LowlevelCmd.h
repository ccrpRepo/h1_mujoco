...
struct LowlevelCmd
{
    ...
    void setTau(Eigen::Matrix<double, 19, 1> tau, Vec2 torqueLimit = Vec2(-2000, 2000))
    {
        for (int i(0); i < 19; ++i)
        {
            if (std::isnan(tau(i)))
    ...
    void setSwingGain(int legID)
    {
        ...
        motorCmd[legID * 5 + 3].mode = 10;
        motorCmd[legID * 5 + 3].Kp = 30; //80
        motorCmd[legID * 5 + 3].Kd = 60; //60

        motorCmd[legID * 5 + 4].mode = 10;
        motorCmd[legID * 5 + 4].Kp = 30; //30
        motorCmd[legID * 5 + 4].Kd = 10; //10
    }
};
